var Theme = {
    ...
    initAdjustments: function($){
        $('a[href^="#"]').click(function(){
            Theme._scrollToElementHelper($(this).attr('href'), 50);
        });

        if($('.slider-home').length > 0){
            //var h = $(window).height() - $('header.header').height();
            var h = $(window).height() - $('header.header').height();
            ...
            if(!Theme.isMobile()){
                $('.slider-home').css('max-height', h + 'px');
                $('.slider-home').css('margin-top', $('header.header').height() + 'px');
    
                setTimeout(function(){
                    var sh = $('.slider-home').height();
                    var slwrap = $('.slick-slide__title-wrap').height();
    
                    if(slwrap < sh){
                        var dif = sh - slwrap;
                        $('.slick-slide__title-wrap').css('margin', (dif/2)+'px auto');
                    }
                }, 100);
            }else{
                $('.slider-home').css('height', h + 'px');
                $('.slider .slick-slide-home').css('height',  h + 'px');
            ...
            var b = '<a class="button button--second added-btn" href="#practice" tabindex="1">OUR SERVICES</a>';

            $('.slick-slide__button').each(function(){
                $(this).prepend(b);
            });

            $('a.added-btn').click(function(){
                Theme._scrollToElementHelper($(this).attr('href'), 50);
            });
        }
        if(!Theme.isMobile()){
            $('#dee-slider .slider').css('margin-top', $('header.header').height() + 'px'); //pages header
        }

        //nav / header scripts

        if(!$('body.home').length){
            var burl = $('#base-url').val() + '/';

            $('a[href="#practice"]').attr('href', burl + '#practice');
            $('a[href="#team"]').attr('href', burl + '#team');

            var inn = $('#header-title-block').width();
            $('#header-title-block').css('margin-left', '-' + (inn/2) + 'px');
        ...
        if($('.bg-black').length > 0){
            setTimeout(function(){
                var bgbh = $('.bg-black').height();
                
                $('.g-map iframe').css('height', (bgbh+100) + 'px');
            }, 100);
        }
    },

    initDrawerScripts: function($){
        if($('.drawer__container').length > 0){

            $('a[href="#drawerbtn"], .drawer__close a').unbind('click');

            $('a[href="#drawerbtn"]').click(function(e){
                e.preventDefault();

                $('.drawer__container').show();

                setTimeout(function(){
                    $('.drawer__inner').css('opacity', '1');
                    $('.drawer__inner').css('width', '100%');
                }, 200);
            });

            $('.drawer__close a').click(function(e){
                e.preventDefault();

                $('.drawer__inner').css('opacity', '0');
                $('.drawer__inner').css('width', '0px');

                setTimeout(function(){
                    $('.drawer__container').hide();
                }, 200);
            });

            $('.drawer__container').click(function(event){
                if (event.target === this) {
                // This code will run when the parent container is clicked
                    $('.drawer__close a').trigger('click');
                }
            });
        }
    },
